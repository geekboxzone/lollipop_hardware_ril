...
static const char* REFERENCE_RIL_VERSION = "RIL_RK_DATA_V3.5"
#ifndef RIL_RELEASE
    " [debug version]"
#endif
#if !ENABLE_DNS_CHECK
...
#define LOG_NDEBUG 0
#define LOG_TAG "RIL_RK_DATA_V3.5"
#include <utils/Log.h>
...
#define ETH_OPERSTATE_PATH "/sys/class/net/lte0/operstate"
#define SERVICE_LTE  "lteup"
#define PPP_ETH_PATH "lte0"
...
static void requestOrSendDataCallList(RIL_Token *t)
{
  ...
  if(!modem_cmp(0x05c6,0x9025,NULL)){
	...
	LOGD("%s ===enter else===",__FUNCTION__);
...
// xxh 部分modem不支持信号查询
	...
	responseV6.GW_SignalStrength.bitErrorRate = response[1];	
	//responseV6.CDMA_SignalStrength.dbm = -1;	
	//responseV6.CDMA_SignalStrength.ecio = -1;	
//	responseV6.EVDO_SignalStrength.dbm = -1;	
	//responseV6.EVDO_SignalStrength.ecio = -1;	
	//responseV6.EVDO_SignalStrength.signalNoiseRatio = -1;	
	responseV6.LTE_SignalStrength.signalStrength=99;	
	responseV6.LTE_SignalStrength.rsrp = 0x7FFFFFFF;	
	responseV6.LTE_SignalStrength.rsrq = 0x7FFFFFFF;	
	responseV6.LTE_SignalStrength.rssnr = 0x7FFFFFFF;	
	responseV6.LTE_SignalStrength.cqi = 0x7FFFFFFF;		
	#endif
	RIL_onRequestComplete(t, RIL_E_SUCCESS, &responseV6, sizeof(RIL_SignalStrength_v6));
...
static void onUnsolicitedSignalStrength()
{
	...
	RIL_onUnsolicitedResponse (RIL_UNSOL_SIGNAL_STRENGTH,&responseV6, sizeof(RIL_SignalStrength_v6));
    at_response_free(p_response);
    return;
...
#define PPP_ERROR 2
    ...
    property_set("dhcp.eth0.result", "");
    property_set("dhcp.eth0.ipaddress", "");
		}
...
//		sleep(3);
		...
		if(!modem_cmp(0x19D2, 0x1244, NULL)||!modem_cmp(0x05c6,0x9025,NULL))
			...
			dhcp_do_request("lte0", &ipaddr, &gateway, &mask, &dns1, &dns2, &server, &lease);
		else		
		dhcp_do_request("eth0", &ipaddr, &gateway, &mask, &dns1, &dns2, &server, &lease);		
        ...
        if (fd >= 0)
        {
            ...
            if(!strncmp(buffer, "up", strlen("up")) || !strncmp(buffer, "unknown", strlen("unknown")))
            {
                ...
                do
                {
                    ...
                    err = property_get("dhcp.eth0.result", result_code, "");
                }
                ...
                property_get("dhcp.eth0.ipaddress", local_ip, "");
                property_get("dhcp.eth0.dns1", local_pdns, "");
                property_get("dhcp.eth0.dns2", local_sdns, "");
				property_get("dhcp.eth0.gateway", local_gateway, "");
				...
				property_set("net.dns1", local_pdns);
				property_set("net.dns2", local_sdns);
				property_set("net.eth0.dns1", local_pdns);
				property_set("net.eth0.dns2", local_sdns);
				property_set("net.eth0.gw", local_gateway);
						}
...
//    asprintf(&conn_script, "/system/bin/chat -v -s -S TIMEOUT 25 ABORT 'BUSY' ABORT 'NO CARRIER' ABORT 'ERROR' ABORT '+CME ERROR:' %s %s CONNECT", s_current_modem->connect_init_script, dialup);
 ...
 if(s_current_modem->radioType==RADIO_TYPE_CDMA || s_current_modem->radioType==RADIO_TYPE_EVDO||!modem_cmp(0x12D1, 0x1D09, "ET127/158/128-2")||!modem_cmp(0x12D1, 0x1D50, "ET302/306"))
    ...
    if(0 == modem_cmp(0x106C, 0x3718, NULL))
        at_emulate_enter();
#endif
	if(!modem_cmp(0x216F, 0x0047, "ALT3100") ||!modem_cmp(0x19D2, 0x1244, "K4201-Z")||!modem_cmp(0x05c6,0x9025, NULL)){
	  ...
	  property_get("dhcp.lte0.gateway", ip_gw, "");
    ...
    response_v6.status = 0;
    response_v6.suggestedRetryTime = -1;
    response_v6.cid = 1;//response[0];
    response_v6.active = 2;
    response_v6.type = "IP";
    response_v6.addresses = response[2];
    response_v6.dnses = NULL;
    response_v6.gateways = ip_gw;
    response_v6.ifname = response[1];
	RLOGD("[%s]: sizeof(RIL_Data_Call_Response_v6)=%d", __FUNCTION__,sizeof(RIL_Data_Call_Response_v6));
    RIL_onRequestComplete(t, RIL_E_SUCCESS, &response_v6, sizeof(RIL_Data_Call_Response_v6));
	}
#else    
	RIL_onRequestComplete(t, RIL_E_SUCCESS, response, sizeof(response));
#endif
    ...
    for (;;) 
    {
        ...
        while  (fd < 0)
        {
           ...
           // LOGD("Wait device...");
            ...
            RLOGD("tty Device id is: %04X/%04X", vid, pid);

            // 根据VID/PID来匹配模块型号
           RLOGD("Searching modem table...");
